"""
Minimal Random Forest Baseline with Aggressive Binning

Intermediate model between simple linear/entanglement-only and full 20-feature RF.
Uses 5-6 conceptually clean, interpretable features.
"""
import json
import numpy as np
import pandas as pd
...
def train_minimal_baseline():
    """
    Train minimal Random Forest with 5-6 interpretable features:
    1. entanglement_stress - core compound metric (Volume × Packing)
    2. n_qubits - circuit width
    3. circuit_depth - temporal complexity
    4. n_2q_gates - entanglement gate count
    5. gate_density - circuit density (gates per qubit per depth)
    """
    print("="*80)
    print("MINIMAL RANDOM FOREST BASELINE")
    print("="*80)
    print()
    ...
    # Minimal, interpretable feature set (selected by correlation)
    minimal_features = [
        'entanglement_stress',  # Core compound metric: Volume × Packing (0.922 corr)
        'density',              # Graph edge density (0.350 corr)
        'n_cz',                 # CZ gate count (0.236 corr)
        'n_barriers',           # Circuit structure/organization (0.204 corr)
        'axis_c_packing',       # Temporal packing component (0.185 corr)
    ]
    
    # Ensure all features exist
    feature_cols = [f for f in minimal_features if f in train_data.columns]
    ...
    # Same difficulty features for aggressive binning
    ...
    # Save artifacts with different names
    output_path = Path("../artifacts")
    output_path.mkdir(exist_ok=True)
    
    with open(output_path / 'minimal_baseline_model.pkl', 'wb') as f:
        pickle.dump(model, f)
    
    with open(output_path / 'minimal_baseline_scaler.pkl', 'wb') as f:
        pickle.dump(scaler, f)
    
    metadata = {
        'model_type': 'random_forest_minimal_baseline',
    ...
    with open(output_path / 'minimal_baseline_metadata.json', 'w') as f:
        json.dump(metadata, f, indent=2)
    
    print(f"Saved artifacts to {output_path}/")
    print("  - minimal_baseline_model.pkl")
    print("  - minimal_baseline_scaler.pkl")
    print("  - minimal_baseline_metadata.json")
    print()
    
    print("="*80)
    print("Model Evolution Comparison:")
    print("  A) Simple linear/entanglement-only: ~0.59 correlation")
    print(f"  B) Minimal RF baseline (this): {test_corr:.3f} correlation")
    print("  C) Full 20-feature RF: ~0.84 correlation")
    print("="*80)
    print()
