...
def extract_polynomial_features(threshold_sweep, degree=5):
    ...
    try:
        ...
        # IMPORTANT <--  0.75 instead of 0.99
        cross_75_threshold = None
        for t_val in np.linspace(log_min, log_max, 1000):
            if poly(t_val) >= 0.75:
                cross_75_threshold = np.expm1(t_val)
                break
        features["poly_cross_75"] = float(cross_75_threshold) if cross_75_threshold else float(max_thresh)
...
def _empty_poly_features(degree):
    return {f"poly_coef_{i}": 0.0 for i in range(degree + 1)} | {
        ...
        "poly_min_fidelity": 0.0, "poly_fidelity_range": 0.0, "poly_cross_75": 0.0
    }
...
def load_data_075(data_path, circuits_dir):
    ...
    for result in results:
        ...
        # 0.75 fidelity target
        min_threshold_075 = None
        for sweep_entry in sorted(threshold_sweep, key=lambda x: x.get("threshold", 999)):
            ...
            if fidelity is not None and isinstance(fidelity, (int, float)) and fidelity >= 0.75:
                min_threshold_075 = threshold
                break
        if min_threshold_075 is None:
            thresholds = [s.get("threshold") for s in threshold_sweep if s.get("threshold") is not None]
            if thresholds:
                min_threshold_075 = max(thresholds)
            else:
                continue
        ...
        features["min_threshold"] = float(min_threshold_075)
        features["forward_runtime"] = float(forward_runtime)
        features["log_runtime"] = float(np.log1p(forward_runtime))
